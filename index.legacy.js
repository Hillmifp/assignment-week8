...
function calculateTotalCashIn(cashFlow) {
    return cashFlow.cashIn.reduce((total, transaction) => total + transaction.amount, 0);
}
// Function untuk menghitung Cash Out
function calculateTotalCashOut(cashFlow) {
    return cashFlow.cashOut.reduce((total, transaction) => total + transaction.amount, 0);
}
...
function updateResults() {
    ...
    if (cashInResultElement && cashOutResultElement) {
        cashInResultElement.textContent = `Total cash in: Rp. ${calculateTotalCashIn(cashFlow)}`;
        cashOutResultElement.textContent = `Total cash out: Rp. ${calculateTotalCashOut(cashFlow)}`;
    }
}
...
function updateResults() {
    ...
    if (cashInResultElement && cashOutResultElement && totalAmountResultElement) {
        ...
        totalAmountResultElement.textContent = `Rp. ${calculateTotalAmount(cashFlow)}`;
    }
}
